-- Track when the user last viewed their matches page.
-- Also bump profiles.updated_at whenever someone's skills change,
-- so get_matches can surface "new since your last visit" cards.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS last_matches_viewed_at TIMESTAMPTZ;

-- Trigger: user_skills insert/delete → update profiles.updated_at for that user
CREATE OR REPLACE FUNCTION public.handle_user_skills_changed()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS on_user_skills_changed ON public.user_skills;
CREATE TRIGGER on_user_skills_changed
  AFTER INSERT OR DELETE ON public.user_skills
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_user_skills_changed();

-- Update get_matches to also return out_updated_at so the client
-- can highlight profiles that are newer than the user's last visit.
